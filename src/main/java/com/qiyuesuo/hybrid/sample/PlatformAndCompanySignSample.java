...
import com.qiyuesuo.hybrid.sample.request.CompanyAuthStatusParam;
import com.qiyuesuo.hybrid.sample.request.CompanyAuthUrlParam;
import com.qiyuesuo.hybrid.sample.request.CompanySealParam;
...
import com.qiyuesuo.hybrid.sample.response.CompanyAuthResult;
 ...
 * 子方法说明：
 * 1、通过文件创建合同：
 * （1）通过文件直接创建合同（Pdf，Html，Word）:{@link PlatformAndCompanySignSample#createByFile()}}
 * （2）通过文件作为模板创建合同（Html，Word）:{@link PlatformAndCompanySignSample#createByTemplate()}}
 * 2、对接方签署(根据自身业务场景选择签署方式)：
 * （1）对接方接口静默签署：{@link PlatformAndCompanySignSample#platfromInterfaceSign(Long)}
 * （2）生成对接方签署页面链接：{@link PlatformAndCompanySignSample#platformPageSign(Long)}
 * 3、公司认证以及签署：
 * （1）生成公司认证链接：{@link PlatformAndCompanySignSample#companyAuthUrl(String, String)}
 * （2）查询公司认证状态：{@link PlatformAndCompanySignSample#companyAuthStatus(String)}
 * （3）生成公司签署页面链接：{@link PlatformAndCompanySignSample#companyPageSign(Long, String, String)}
 * 4、完成合同：{@link PlatformAndCompanySignSample#completeContract(Long)}
 * 5、生成合同预览链接：{@link PlatformAndCompanySignSample#viewContract(Long)}
 * 6、下载合同文件：{@link PlatformAndCompanySignSample#downlodContract(Long)}
 */
public class PlatformAndCompanySignSample implements BaseSample{

	private HybridClient hybridClient;
	
	public PlatformAndCompanySignSample(HybridClient client) {
	...
	public void run() throws Exception {
		...
		/*
		 * 公司签署
		 *（1）生成公司认证页面，获取认证凭证，完成并通过公司认证
		 *（2）通过认证凭证生成公司签署页面，开始签署
		 * 注：对应公司认证完成后，对接方可持久化认证凭证，后续相同公司再次进行签署时，可跳过认证步骤，直接生成公司签署页面
		 */
		String authId = this.companyAuthUrl("测试签署公司", "17600000000");
		if (this.companyAuthStatus(authId).equals(2)) {
			String base64 = this.companySeal("测试签署公司");
			String companyUrl = this.companyPageSign(documentId,authId,base64);
		}else {
	...
	private String platformPageSign(Long documentId) {
		...
		if (!responseObject.getCode().equals(ResponseCode.SUCCESS.getCode())) {
			throw new HybridException("生成合同签署链接失败，错误码：" + responseObject.getCode() + "，错误原因：" + responseObject.getMessage());
		}
		PageBean bean = responseObject.getResult();
		System.out.println("生成合同签署链接成功，签署链接：" + bean.getUrl());
		return bean.getUrl();
	}
	 ...
	 * @param contact	待认证公司联系方式
	 * @return
	 */
	private String companyAuthUrl(String companyName,String contact) {
		CompanyAuthUrlParam urlParam = new CompanyAuthUrlParam();
		ApplicantBean applicantBean = new ApplicantBean();
		applicantBean.setContact(contact);
		applicantBean.setContactType("MOBILE");
		urlParam.setApplicant(applicantBean);
		urlParam.setCorpName(companyName);
		// 请求远程服务器
		String pageResponse = hybridClient.doService(RequestPathProperties.COMPANY_AUTH_PAGE.getPath(), urlParam.getHttpParameter());
		// 解析返回结果，并获取documentid
		HybridResponse<AuthUrl> responseObject = JSONUtils.toHybridResponse(pageResponse, AuthUrl.class);
		if (!responseObject.getCode().equals(ResponseCode.SUCCESS.getCode())) {
			throw new HybridException("生成公司认证链接失败，错误码：" + responseObject.getCode() + "，错误原因：" + responseObject.getMessage());
		}
	 ...
	 * 查询企业认证状态
	 */
	private Integer companyAuthStatus(String authId) {
		CompanyAuthStatusParam statusParam = new CompanyAuthStatusParam();
		statusParam.setAuthId(authId);
		String resultResponse = hybridClient.doService(RequestPathProperties.COMPANY_AUTH_STATUS.getPath(), statusParam.getHttpParameter());
		HybridResponse<CompanyAuthResult> responseObject = JSONUtils.toHybridResponse(resultResponse, CompanyAuthResult.class);
		if (!responseObject.getCode().equals(ResponseCode.SUCCESS.getCode())) {
			throw new HybridException("查询个人认证结果失败，错误码：" + responseObject.getCode() + "，错误原因：" + responseObject.getMessage());
		}
		CompanyAuthResult result = responseObject.getResult();
		System.out.println("查询个人认证状态成功，认证信息：" + resultResponse);
		return result.getStatus();
	}

	/**
	 * 创建公章外观
	 * @param companyName 生成公章的公司名称
	 * @return
	 */
	private String companySeal(String companyName) {
		CompanySealParam sealParam = new CompanySealParam();
		sealParam.setContent(companyName);
		sealParam.setSealSpec("CIRCULAR_42");
		sealParam.setSealCategory("enterprise");
		String resultResponse = hybridClient.doService(RequestPathProperties.GERENATE_COMPANY_SEAL.getPath(), sealParam.getHttpParameter());
		HybridResponse<String> responseObject = JSONUtils.toHybridResponse(resultResponse, String.class);
		if (!responseObject.getCode().equals(ResponseCode.SUCCESS.getCode())) {
			throw new HybridException("生成公司公章外观失败，错误码：" + responseObject.getCode() + "，错误原因：" + responseObject.getMessage());
		}
		System.out.println("生成公章外观成功，公司名称：" + companyName);
	...
	/**
	 * 生成公司签署页面
	 * 此处未指定签署位置，且设置校验方式为验证码校验
	 * 对接方可根据自身情况，根据接口清单文档，调整接入参数
	 */
	private String companyPageSign(Long documentId, String authId,String base64) {
		PersonalPageSignParam pageSignParam = new PersonalPageSignParam();
		pageSignParam.setDocumentId(documentId);
		pageSignParam.setSigner(new SignerBean(authId, "COMPANY"));
		pageSignParam.setSignValidateMethods(Arrays.asList("PIN"));
		pageSignParam.setAppearance(new SignAppearanceBean(base64, "CIRCULAR_42"));
		...
		if (!responseObject.getCode().equals(ResponseCode.SUCCESS.getCode())) {
			throw new HybridException("生成公司签署链接失败，错误码：" + responseObject.getCode() + "，错误原因：" + responseObject.getMessage());
		}
		PageBean url = responseObject.getResult();
		System.out.println("生成公司签署链接成功，签署链接：" + url.getUrl());
		return url.getUrl();
	}
	...
	private String viewContract(Long documentId) {
		...
		PageBean bean = responseObject.getResult();
		System.out.println("生成合同预览链接成功，链接：" + bean.getUrl());
		return bean.getUrl();
	}
